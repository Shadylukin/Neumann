...
> **See Also:**
> - [Neumann Server API](neumann-server.md) -- server that this SDK connects to
> - [Neumann Client API (Rust)](neumann-client.md) -- Rust SDK with same capabilities
> - [Python SDK API](neumann-py.md) -- Python SDK with same API design
...
| --- | --- |
| `rowToObject` | Convert Row to plain JavaScript object |
| `nodeToObject` | Convert Node to `{ id, label, properties }` |
| `edgeToObject` | Convert Edge to `{ id, type, source, target, properties }` |
...
Proto conversion functions are used internally but exported for custom
integrations.
...
async function main() {
  ...
  try {
    ...
    if (isRowsResult(result)) {
      console.log('Products over $10:', result.rows.map(rowToObject));
    }
