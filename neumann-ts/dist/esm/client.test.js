...
const mockGrpcClient = {
    Execute: vi.fn((request, metadata, callback) => {
    ...
    ExecuteBatch: vi.fn((request, metadata, callback) => {
        callback(null, mockBatchResponse);
    }),
    ExecutePaginated: vi.fn((request, metadata, callback) => {
        callback(null, mockPaginatedResponse);
    }),
    CloseCursor: vi.fn((request, metadata, callback) => {
...
describe('NeumannClient', () => {
    ...
    describe('connect', () => {
        ...
        it('should throw ConnectionError when gRPC fails', async () => {
            ...
            grpcModule.loadProto = vi.fn().mockRejectedValue(new Error('Connection refused'));
    ...
    describe('connectWeb', () => {
        ...
        it('should throw ConnectionError when gRPC-Web fails', async () => {
            ...
            grpcWeb.GrpcWebClientBase = vi.fn().mockImplementation(() => {
    ...
    describe('executeBatch', () => {
        it('should throw if not connected', async () => {
            const client = await NeumannClient.connect('localhost:50051');
            client.close();
            await expect(client.executeBatch(['SELECT 1', 'SELECT 2'])).rejects.toThrow(ConnectionError);
        });
...
describe('Conversion Utilities', () => {
    ...
    describe('edgeToObject', () => {
        it('should convert edge to plain object', () => {
            ...
            expect(obj['properties']['weight']).toBe(0.5);
        });
    });
});
...
describe('Pagination', () => {
    describe('executePaginated', () => {
        it('should throw if not connected', async () => {
            const client = await NeumannClient.connect('localhost:50051');
            client.close();
            await expect(client.executePaginated('SELECT * FROM users')).rejects.toThrow(ConnectionError);
        });
...
describe('New Type Guards', () => {
    ...
    describe('isAggregateResult', () => {
        it('should return true for aggregate result', () => {
            expect(isAggregateResult({ type: 'aggregate', value: { type: 'count', value: 10 } })).toBe(true);
        });
        it('should return false for other types', () => {
    ...
    describe('isChainQueryResult', () => {
        it('should return true for chain query result', () => {
            expect(isChainQueryResult({ type: 'chain', result: { type: 'height', value: { height: 10 } } })).toBe(true);
        });
        it('should return false for other types', () => {
...
describe('Validation Utilities', () => {
    ...
    describe('safeIdsToStrings', () => {
        ...
        it('should reject array with unsafe integers', () => {
            expect(() => safeIdsToStrings([1, Number.MAX_SAFE_INTEGER + 1, 3])).toThrow(InvalidArgumentError);
        });
    });
});
...
describe('Copy Helper Functions', () => {
    describe('copyRowValues', () => {
        it('should create mutable copy of row values', () => {
            const row = { values: new Map([['id', intValue(1)]]) };
...
describe('Safe ID Conversions', () => {
    it('should handle unsafe page rank node IDs', () => {
        expect(() => convertProtoPageRankItem({ nodeId: Number.MAX_SAFE_INTEGER + 1, score: 0.5 })).toThrow(InvalidArgumentError);
    });
    it('should handle unsafe centrality node IDs', () => {
        expect(() => convertProtoCentralityItem({ nodeId: Number.MAX_SAFE_INTEGER + 1, score: 0.5 })).toThrow(InvalidArgumentError);
    });
    it('should handle unsafe community node IDs', () => {
        expect(() => convertProtoCommunityItem({ nodeId: Number.MAX_SAFE_INTEGER + 1, communityId: 1 })).toThrow(InvalidArgumentError);
    });
    it('should handle unsafe community member IDs', () => {
        expect(() => convertProtoCommunityMemberList({
            communityId: 1,
            memberNodeIds: [1, Number.MAX_SAFE_INTEGER + 1],
        })).toThrow(InvalidArgumentError);
    });
    it('should handle unsafe binding value node ID', () => {
        expect(() => convertProtoBindingValue({ node: { id: Number.MAX_SAFE_INTEGER + 1, label: 'Person' } })).toThrow(InvalidArgumentError);
    });
    it('should handle unsafe binding value edge IDs', () => {
        expect(() => convertProtoBindingValue({
