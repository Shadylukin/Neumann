 ...
 */
export function validateIntValue(n: number, context?: string): number {
  ...
  if (!Number.isSafeInteger(n)) {
    ...
    throw new InvalidArgumentError(
      `Integer value ${n} is outside safe range [-2^53+1, 2^53-1]${ctx}`
    );
  }
  return n;
}
 ...
 * @throws InvalidArgumentError if validation fails.
 */
export function validateFloatValue(n: number, context?: string): number {
  if (!Number.isFinite(n)) {
    const ctx = context ? ` (${context})` : '';
 ...
 * @throws InvalidArgumentError if validation fails.
 */
export function validateStringValue(s: string, context?: string): string {
  if (s.length > MAX_STRING_LENGTH) {
    const ctx = context ? ` (${context})` : '';
    throw new InvalidArgumentError(
      `String value exceeds maximum length of ${MAX_STRING_LENGTH} bytes${ctx}`
    );
  }
  return s;
}
 ...
 * @throws InvalidArgumentError if validation fails.
 */
export function validateBytesValue(b: Uint8Array, context?: string): Uint8Array {
  if (b.length > MAX_BYTES_LENGTH) {
    const ctx = context ? ` (${context})` : '';
    throw new InvalidArgumentError(
      `Bytes value exceeds maximum length of ${MAX_BYTES_LENGTH} bytes${ctx}`
    );
  }
  return b;
}
 ...
 */
export function safeIdToString(id: number, context?: string): string {
  if (!Number.isSafeInteger(id)) {
    const ctx = context ? ` for ${context}` : '';
    throw new InvalidArgumentError(
      `ID value ${id} is outside safe integer range${ctx}`
    );
  }
  return String(id);
}
 ...
 */
export function safeIdsToStrings(ids: number[], context?: string): string[] {
