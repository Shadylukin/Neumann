...
describe('retry', () => {
  ...
  describe('isRetryable', () => {
    ...
    it('returns true for UNAVAILABLE status code', () => {
      expect(
        isRetryable({ code: RETRYABLE_STATUS_CODES.UNAVAILABLE }, DEFAULT_RETRY_CONFIG)
      ).toBe(true);
    });

    it('returns true for DEADLINE_EXCEEDED status code', () => {
      expect(
        isRetryable({ code: RETRYABLE_STATUS_CODES.DEADLINE_EXCEEDED }, DEFAULT_RETRY_CONFIG)
      ).toBe(true);
    });
