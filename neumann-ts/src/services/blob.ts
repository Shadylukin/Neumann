...
export class BlobClient {
  ...
  async uploadBlob(
    ...
    return new Promise((resolve, reject) => {
      const call = this.client.Upload(
        this.metadata,
        (err: grpc.ServiceError | null, response: BlobUploadResponse) => {
          if (err) {
   ...
   */
  async downloadBlobFull(artifactId: string): Promise<Buffer> {
   ...
   */
  async deleteBlob(artifactId: string): Promise<boolean> {
    return new Promise((resolve, reject) => {
      this.client.Delete(
        { artifactId },
        this.metadata,
        (err: grpc.ServiceError | null, response: BlobDeleteResponse) => {
          if (err) {
            reject(this.handleError(err));
            return;
          }
   ...
   */
  async getBlobMetadata(artifactId: string): Promise<ArtifactMetadata> {
    return new Promise((resolve, reject) => {
      this.client.GetMetadata(
        { artifactId },
        this.metadata,
        (err: grpc.ServiceError | null, response: ArtifactInfo) => {
          if (err) {
            reject(this.handleError(err));
            return;
          }
