...
fn bench_quantize_int8(c: &mut Criterion) {
    ...
    println!("  quantize_int8_768d peak RAM: {} KB", PEAK_ALLOC.peak_usage_as_kb());
}
...
fn bench_quantize_binary(c: &mut Criterion) {
    ...
    println!("  quantize_binary_768d peak RAM: {} KB", PEAK_ALLOC.peak_usage_as_kb());
}
...
fn bench_delta_compress(c: &mut Criterion) {
    ...
    println!("  compress_ids_10k peak RAM: {} KB", PEAK_ALLOC.peak_usage_as_kb());
}
...
fn bench_delta_decompress(c: &mut Criterion) {
    ...
    println!("  decompress_ids_10k peak RAM: {} KB", PEAK_ALLOC.peak_usage_as_kb());
}
...
fn bench_rle_encode(c: &mut Criterion) {
    ...
    println!("  rle_encode_100k peak RAM: {} KB", PEAK_ALLOC.peak_usage_as_kb());
}
...
fn bench_rle_decode(c: &mut Criterion) {
    ...
    println!("  rle_decode_100k peak RAM: {} KB", PEAK_ALLOC.peak_usage_as_kb());
}
