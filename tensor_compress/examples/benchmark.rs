...
fn main() {
    let dims: [u16; 5] = [64, 256, 768, 1536, 4096];
    ...
    for dim in dims {
        let vector: Vec<f32> = (0..dim).map(|i| (f32::from(i) * 0.01).sin()).collect();
        ...
        let decompose_us = start.elapsed().as_secs_f64() * 1_000_000.0 / f64::from(iterations);
        let tt = tt.unwrap();
        ...
        let reconstruct_us = start.elapsed().as_secs_f64() * 1_000_000.0 / f64::from(iterations);
        ...
        let similarity_us = start.elapsed().as_secs_f64() * 1_000_000.0 / f64::from(iterations);
    ...
    let batch_dim: u16 = 768;
    let vectors: Vec<Vec<f32>> = (0_u16..1000)
        .map(|i| {
            (0..batch_dim)
                .map(|j| (f32::from(i) * f32::from(j) * 0.01).sin())
                .collect()
        })
        .collect();
    let config = TTConfig::for_dim(usize::from(batch_dim)).unwrap();
    ...
    println!(
        "Batch decompose 1000x{}: {:.0} ms ({:.1} µs/vector)",
        batch_dim,
        batch_ms,
        batch_ms * 1000.0 / 1000.0
    );

    let start = Instant::now();
    let _ = tt_cosine_similarity_batch(&tts[0], &tts[1..]).unwrap();
    let batch_sim_us = start.elapsed().as_secs_f64() * 1_000_000.0;
    println!(
        "Batch similarity 1 vs 999: {:.0} µs ({:.2} µs/comparison)",
        batch_sim_us,
        batch_sim_us / 999.0
    );
    ...
    // Throughput
    println!("\n=== Throughput ===\n");
    let throughput_dim: u16 = 768;
    let config = TTConfig::for_dim(usize::from(throughput_dim)).unwrap();
    let vector: Vec<f32> = (0..throughput_dim)
