...
fn bench_parse(c: &mut Criterion) {
    ...
    let queries = [
        ...
        ("create_table", "CREATE TABLE users (id, name, email, age, active)"),
        ("node", "NODE person {name: 'Alice', age: 30, email: 'alice@example.com'}"),
        ("edge", "EDGE person:1 friend person:2 {since: 2020, strength: 0.8}"),
        ("neighbors", "NEIGHBORS person:1 friend OUTGOING"),
        ("path", "PATH person:1 TO person:100 VIA friend"),
        ("find_node", "FIND NODE WHERE label = 'person' AND age > 21"),
...
fn bench_batch_parse(c: &mut Criterion) {
    ...
    let queries: Vec<&str> = vec![
        ...
        "EDGE person:1 friend person:2",
        "PATH person:1 TO person:2",
